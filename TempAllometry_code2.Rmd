...
male.start <- ggplot(dat[male.rows,], aes(y=Time2gonadogenesis,x=SMgroup,color=TEMPgroup, fill=TEMPgroup,shape=TEMPgroup))+
  ...
  ylab("Days") + xlab("Size manipulation group")+labs(shape=leg.title,color=leg.title,fill=leg.title,title="Male sexual development time:")
...
male.fecundity <- ggplot(dat[male.rows,], aes(y=maxTestes,x=SMgroup,color=TEMPgroup, fill=TEMPgroup,shape=TEMPgroup))+
  ...
  scale_fill_manual(values=c("12"="red","8"="blue"))+
  scale_shape_manual(values=c("12"=24, "8"=21))+
  ylab("No. testes") + xlab("Size manipulation group")+labs(shape=leg.title,color=leg.title,fill=leg.title,title="Male fecundity:")
...
female.start <- ggplot(dat[female.rows,], aes(y=Time2gonadogenesis,x=SMgroup,color=TEMPgroup, fill=TEMPgroup,shape=TEMPgroup))+
  ...
  scale_fill_manual(values=c("12"="red","8"="blue"))+
  scale_shape_manual(values=c("12"=24, "8"=21))+
  ylab("Days") + xlab("Size manipulation group")+labs(shape=leg.title,color=leg.title,fill=leg.title,title="Female sexual development time:")
...
female.fecundity <- ggplot(dat[female.rows,], aes(y=sumEggs,x=SMgroup,color=TEMPgroup, fill=TEMPgroup,shape=TEMPgroup))+
  ...
  scale_fill_manual(values=c("12"="red","8"="blue"))+
  scale_shape_manual(values=c("12"=24, "8"=21))+
  ylab("No. eggs") + xlab("Size manipulation group")+labs(shape=leg.title,color=leg.title,fill=leg.title, title="Female fecundity:")
```
...
male.survival<-ggplot(filter(surv, Sex=="male"&Survival==1&Strain%in%c("C2/7","M83/4")), aes(y=prop, x=SMgroup,color=TEMPgroup,fill=TEMPgroup,group=TEMPgroup,shape=TEMPgroup))+
  ...
  scale_shape_manual(values=c("12"=24, "8"=21))+
  xlab("Size manipulation group")+ylab("Prop. surviving")+labs(title="Male survival:")
...
female.survival<-ggplot(filter(surv, Survival==1&Sex=="female"&Strain%in%c("M26/9/10","X11/14")), aes(y=prop, x=SMgroup,color=TEMPgroup,fill=TEMPgroup,group=TEMPgroup,shape=TEMPgroup))+
  ...
  stat_summary(aes(group=TEMPgroup),fun="mean",geom="line",position=position_dodge(0.9),linewidth=1.5)+
  scale_shape_manual(values=c("12"=24, "8"=21))+
  xlab("Size manipulation group")+ylab("Prop. surviving")+labs(title="Female survival:")
...
male.sizechange<-ggplot(dat2[dat2$Strain%in%c("C2/7","M83/4"),], aes(y=w2Size/postSize,color=TEMPgroup, x=SMgroup, fill=TEMPgroup, shape=TEMPgroup))+facet_wrap(~Strain)+theme_bw()+
  ...
  scale_color_manual(values=c("12"="red","8"="blue"))+
  scale_fill_manual(values=c("12"="red","8"="blue"))+
  xlab("Size manipulation group")+ylab("Size change")+labs(shape=leg.title,color=leg.title,fill=leg.title,title="Male compensatory growth:")
...
female.sizechange<-ggplot(dat2[dat2$Strain%in%c("X11/14","M26/9/10"),], aes(y=w2Size/postSize,color=TEMPgroup, x=SMgroup, fill=TEMPgroup, shape=TEMPgroup))+facet_wrap(~Strain)+theme_bw()+
  ...
  scale_color_manual(values=c("12"="red","8"="blue"))+
  scale_fill_manual(values=c("12"="red","8"="blue"))+
  xlab("Size manipulation group")+ylab("Size change")+labs(shape=leg.title,color=leg.title,fill=leg.title,title="Female compensatory growth:")
...
p1 <- "~/hidra/2023/TempAllometry/MS/submission1/graph/start.png"
p2 <- "~/hidra/2023/TempAllometry/MS/submission1/graph/8fok.png"
p3 <- "~/hidra/2023/TempAllometry/MS/submission1/graph/12fok.png"
...
g8 <- ggplot(g.temp2[g.temp2$Temperature==8,],aes(x=week, y=sex.props,shape=Size,linetype=Size)) + 
  geom_line(color="blue")+geom_point(color="blue")+theme_bw()+xlab("Week")+ylab("Prop. reproducing")+
...
g12 <- ggplot(g.temp2[g.temp2$Temperature==12,],aes(x=week, y=sex.props,shape=Size,linetype=Size)) + 
  geom_line(color="red")+geom_point(color="red")+theme_bw()+xlab("Week")+ylab("Prop. reproducing")+
...
ragg::agg_tiff(filename="~/hidra/2023/TempAllometry/MS/plain.tiff",
               width=115,height=115,units="mm",res=300,scaling=0.6)
ggarrange(male.fecundity + theme(legend.title.align = 1), 
          male.survival, female.fecundity, female.survival, ncol=2, nrow=2,common.legend = T)
dev.off()

ggarrange(male.start+theme(plot.title=element_text(size=12),
        legend.spacing.x = unit(0.8, 'cm')),
          female.start+theme(plot.title=element_text(size=12),
        legend.spacing.x = unit(0.8, 'cm')), 
        nrow=1, ncol=2, common.legend = T, labels="AUTO")
ggsave(file="~/hidra/2023/TempAllometry/MS/fig_s1.tiff",width=174,height=121,units="mm",bg="white")

ggarrange(male.sizechange+theme(plot.title=element_text(size=12),
        legend.spacing.x = unit(0.8, 'cm')),
          female.sizechange+theme(plot.title=element_text(size=12),
        legend.spacing.x = unit(0.8, 'cm')),
          nrow=1, ncol=2, common.legend = T, labels="AUTO")
ggsave(file="~/hidra/2023/TempAllometry/MS/fig_s2.tiff",width=174,height=121,units="mm",bg="white")+
...
mFecundity.males.int.poisson <- glmmTMB(maxTestes~SMgroup*TEMPgroup+Strain+(1|PairID), data=dat[male.rows,],family="poisson")
mFecundity.males.int.negbin1 <- glmmTMB(maxTestes~SMgroup*TEMPgroup+Strain+(1|PairID), data=dat[male.rows,],family="nbinom1")
mFecundity.males.int.negbin2 <- glmmTMB(maxTestes~SMgroup*TEMPgroup+Strain+(1|PairID), data=dat[male.rows,],family="nbinom2")

mFecundity.males.add.poisson <- glmmTMB(maxTestes~SMgroup+TEMPgroup+Strain+(1|PairID), data=dat[male.rows,],family="poisson")
anova(mFecundity.males.int.poisson, mFecundity.males.add.poisson) # Chisq: 5.913 / p = 0.015
plot(simulateResiduals(mFecundity.males.int.poisson)) # model diagnostics not as good
...
anova(mSurvival.males.int, mSurvival.males.add) # Chisq: 4.748 / p = 0.040
summary(mSurvival.males.int)
plot(simulateResiduals(mSurvival.males.int)) # model diagnostics OK
```

## Male selal development time
...
## Strain-specific analyzes

```{r males}
library(modelsummary)
testes <- glmmTMB(maxTestes~SMgroup*TEMPgroup*Strain+(1|PairID), 
                   data=dat[male.rows,])
surv <- glmmTMB(Survival~SMgroup*TEMPgroup*Strain+(1|PairID), 
                   data=dat[male.rows,],family="binomial")
sexdev <- glmmTMB(as.numeric(Time2gonadogenesis)~SMgroup*TEMPgroup*Strain+(1|PairID), data=dat[male.rows,])
compgrowth <- glmmTMB(log(w2Size/postSize)~SMgroup*TEMPgroup*Strain+(1|PairID), data=dat2[dat2$Strain%in%c("C2/7","M83/4"),])

models_males <- list("No. testes" = testes, 
                     "Survival" = surv, 
                     "Sexual development time" = sexdev, 
                     "Compensatory growth" = compgrowth)

modelsummary(models_males, estimate="{estimate} ({std.error}){stars}",fmt=2,
             output="~/hidra/2023/TempAllometry/MS/revision/males_interaction.docx")

```

```{r females}
eggs <- glmmTMB(sumEggs~SMgroup*TEMPgroup*Strain+(1|PairID), 
                   data=dat[female.rows,],family="nbinom2")
surv <- glmmTMB(Survival~SMgroup*TEMPgroup*Strain+(1|PairID), 
                   data=dat[female.rows,],family="binomial")
sexdev <- glmmTMB(as.numeric(Time2gonadogenesis)~SMgroup*TEMPgroup*Strain+(1|PairID), data=dat[female.rows,])
dat2$Strain <- factor(dat2$Strain,levels=levels(dat$Strain))
compgrowth <- glmmTMB(log(w2Size/postSize)~SMgroup*TEMPgroup*Strain+(1|PairID), data=dat2[dat2$Strain%in%c("X11/14","M26/9/10"),])

models_females <- list("No. eggs" = eggs, 
                     "Survival" = surv, 
                     "Sexual development time" = sexdev, 
                     "Compensatory growth" = compgrowth)

modelsummary(models_females, estimate="{estimate} ({std.error}){stars}",fmt=2,
             output="~/hidra/2023/TempAllometry/MS/revision/females_interaction.docx")
